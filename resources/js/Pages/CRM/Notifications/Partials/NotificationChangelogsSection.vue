...
const notificationLogEntriesData = reactive({});
let originalLogEntriesData = null;

onMounted(async () => {
  try {
    const notificationLogEntriesResponse = await axios.get(route('notifications.getNotificationLogEntries', props.selectedRowData.id));
    notificationLogEntriesData.value = notificationLogEntriesResponse.data;
    originalLogEntriesData = notificationLogEntriesData.value;
...
const filterByToday = () => {
  const today = dayjs();
  notificationLogEntriesData.value = originalLogEntriesData.filter((log) =>
...
const showAll = () => {
  notificationLogEntriesData.value = originalLogEntriesData;
};
...
<template>
    <div class="input-group p-8 rounded-xl">
        ...
        <div class="container hidden-scrollable max-h-[500px] pt-2">
            <div class="w-full flex justify-center" v-if="!notificationLogEntriesData.value || notificationLogEntriesData.value.length === 0">
                <h3 class="font-semibold text-gray-200"> No records found for this notification currently. </h3>
            </div>
            <div v-for="(log, index) in notificationLogEntriesData.value" :key="index">
