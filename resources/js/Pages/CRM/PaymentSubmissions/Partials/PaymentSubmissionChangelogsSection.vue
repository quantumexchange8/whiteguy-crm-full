...
const paymentSubmissionLogEntriesData = reactive({});
let originalLogEntriesData = null;

onMounted(async () => {
  try {
    const paymentSubmissionLogEntriesResponse = await axios.get(route('payment-submissions.getPaymentSubmissionLogEntries', props.selectedRowData.id));
    paymentSubmissionLogEntriesData.value = paymentSubmissionLogEntriesResponse.data;
    originalLogEntriesData = paymentSubmissionLogEntriesData.value;
...
const filterByToday = () => {
  const today = dayjs();
  paymentSubmissionLogEntriesData.value = originalLogEntriesData.filter((log) =>
...
const showAll = () => {
  paymentSubmissionLogEntriesData.value = originalLogEntriesData;
};
...
<template>
    <div class="input-group p-8 rounded-xl">
        ...
        <div class="container hidden-scrollable max-h-[500px] pt-2">
            <div class="w-full flex justify-center" v-if="!paymentSubmissionLogEntriesData.value || paymentSubmissionLogEntriesData.value.length === 0">
                <h3 class="font-semibold text-gray-200"> No records found for this payment submission currently. </h3>
            </div>
            <div v-for="(log, index) in paymentSubmissionLogEntriesData.value" :key="index">
                        ...
                        <div class="bg-gray-700 dark:bg-gray-700/70 col-start-2 col-end-13 p-4 rounded-xl mb-4 mr-auto shadow-md w-full">
                            ...
                            <div v-if="log.changes && Object.keys(log.changes).length > 0">
                                <Disclosure v-slot="{ open }">
                                    ...
                                    >
                                        Payment Submission Log Entries 
