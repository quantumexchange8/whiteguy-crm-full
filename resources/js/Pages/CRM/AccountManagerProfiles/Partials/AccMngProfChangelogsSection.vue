...
const accMngProfLogEntriesData = reactive({});
let originalLogEntriesData = null;

onMounted(async () => {
  try {
    const accMngProfLogEntriesResponse = await axios.get(route('account-manager-profiles.getAccontManagerProfileLogEntries', props.selectedRowData.id));
    accMngProfLogEntriesData.value = accMngProfLogEntriesResponse.data;
    originalLogEntriesData = accMngProfLogEntriesData.value;
...
const filterByToday = () => {
  const today = dayjs();
  accMngProfLogEntriesData.value = originalLogEntriesData.filter((log) =>
...
const showAll = () => {
  accMngProfLogEntriesData.value = originalLogEntriesData;
};
...
<template>
    <div class="input-group p-8 rounded-xl">
        ...
        <div class="container hidden-scrollable max-h-[500px] pt-2">
            ...
            <div class="w-full flex justify-center" v-if="!accMngProfLogEntriesData.value || accMngProfLogEntriesData.value.length === 0">
                <h3 class="font-semibold text-gray-200"> No records found for this user currently. </h3>
            </div>
            <div v-for="(log, index) in accMngProfLogEntriesData.value" :key="index">
                        ...
                        <div class="bg-gray-700 dark:bg-gray-700/70 col-start-2 col-end-13 p-4 rounded-xl mb-4 mr-auto shadow-md w-full">
                            ...
                            <div v-if="log.changes && Object.keys(log.changes).length > 0">
                                <Disclosure v-slot="{ open }">
                                    ...
                                    >
                                        Account Manager Profile Log Entries 
