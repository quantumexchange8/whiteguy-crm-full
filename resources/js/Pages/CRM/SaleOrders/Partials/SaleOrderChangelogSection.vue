...
const saleOrderLogEntriesData = reactive({});
let originalLogEntriesData = null;

onMounted(async () => {
  try {
    const saleOrderLogEntriesResponse = await axios.get(route('sale-orders.getSaleOrderLogEntries', props.selectedRowData.id));
    saleOrderLogEntriesData.value = saleOrderLogEntriesResponse.data;
    originalLogEntriesData = saleOrderLogEntriesData.value;
...
const filterByToday = () => {
  const today = dayjs();
  saleOrderLogEntriesData.value = originalLogEntriesData.filter((log) =>
...
const showAll = () => {
  saleOrderLogEntriesData.value = originalLogEntriesData;
};
...
<template>
    <div class="input-group p-8 rounded-xl">
        ...
        <div class="container hidden-scrollable max-h-[500px] pt-2">
            ...
            <div class="w-full flex justify-center" v-if="!saleOrderLogEntriesData.value || saleOrderLogEntriesData.value.length === 0">
                <h3 class="font-semibold text-gray-200"> No records found for this user currently. </h3>
            </div>
            <div v-for="(log, index) in saleOrderLogEntriesData.value" :key="index">
